...
const introModal = () => {
  ...
  if ($('#game').data('game') === null) {
    document
      .querySelector('#helpModal-tutorial')
      .addEventListener('click', e => {
        tutorialGuide();
      });
  }
...
const tutorial8 = () => {
  ...
  document.querySelectorAll('.pirate').forEach(el => {
    el.querySelectorAll('div').forEach(ell => {
      arr1.push(ell);
    });
  });
  console.log(arr1);
  ...
  document.getElementById('resolve-fight').style.border = '5px solid blue';
  instructions.innerHTML =
  ...
  setTimeout(() => {
    modal.addEventListener('click', e => {
      ...
      goals.innerHTML = noGoal.outerHTML;
